...
local PIPE_DIAMETER = 0.03175 -- [m]
local PIPE_CROSS_SECTION_AREA = math.pi * PIPE_DIAMETER ^ 2 / 4 -- [m²]
local HOSE_LENGTH = 0.62 -- [m]

--- @class Vehicle
--- @field private length number
--- @field public brakePipe Pipe
--- @field public feedPipe Reservoir
...
function Vehicle:new(length)
    ...
    local instance = {
        length = length,
        brakePipe = Pipe:new(length + 2 * HOSE_LENGTH, PIPE_DIAMETER),
        feedPipe = nil,
        brakeValve = nil,
        distributor = nil,
        accelerator = nil,
...
function Vehicle:addFeedPipe(mainResCapacity)
    self.feedPipe = Reservoir:new(self.length * PIPE_CROSS_SECTION_AREA + (mainResCapacity or 0))
end
