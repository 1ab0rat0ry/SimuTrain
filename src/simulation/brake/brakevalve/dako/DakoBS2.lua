...
function DakoBS2:updateDistributorMechanism(deltaTime, feedPipe, brakePipe)
    deltaTime = 0.1 * math.min(0.1, deltaTime)
    ...
    for _ = 1, 10 do
        ...
        local newPosition = 0.01 * force
        local positionDelta = newPosition - self.distributorValve

        if math.abs(positionDelta) <= FRICTION then positionDelta = 0
        else positionDelta = positionDelta - MathUtil.sign(positionDelta) * FRICTION
        end
        positionDelta = MathUtil.clamp(positionDelta, -2, 2)

        self.distributorValve = MathUtil.clamp(self.distributorValve + 2 * positionDelta * deltaTime, -1, 1)

        if math.abs(self.distributorValve) < 1e-3 then
...
function DakoBS2:updateOvercharge(deltaTime)
    ...
    self.overchargeRes:equalize(self.brakePipeChamber, 4.212e-6 * Call("GetControlValue", "Overcharge", 0), deltaTime)
end
